...
export function TabBar({ paneId, tabSessionIds, activeTabSessionId, isFocused }: TabBarProps) {
  ...
  const handleDragStart = (e: DragEvent, sessionId: string) => {
    e.dataTransfer.setData("application/x-pane-tab", JSON.stringify({ fromPaneId: paneId, sessionId }))
    e.dataTransfer.effectAllowed = "move"
  }

  const handleDragOver = (e: DragEvent) => {
    const hasTab = e.dataTransfer.types.includes("application/x-pane-tab")
    const hasSidebar = e.dataTransfer.types.includes("application/x-sidebar-session")
  ...
  const handleDrop = (e: DragEvent) => {
    ...
    const tabData = e.dataTransfer.getData("application/x-pane-tab")
    if (tabData) {
      try {
    ...
    const sidebarData = e.dataTransfer.getData("application/x-sidebar-session")
    if (sidebarData) {
      try {
    ...
    >
      ...
      <div className="flex items-center shrink-0 px-1.5 pb-1 gap-0.5">
        <button
          ...
          disabled={layout.panes.length >= 4 || !activeTabSessionId}
